...
class ResNet(nn.Module):
    ...
    def __init__(self, block, blocks_num, in_channels=3, num_classes=1000, include_top=True, groups=1,
                 width_per_group=64):
    ...
    def _make_layer(self, block, channel, block_num, stride=1):
        ...
        layers = [block(self.in_channel, channel, downsample=downsample, stride=stride, groups=self.groups,
                        width_per_group=self.width_per_group)]  # 定义1个空列表
    ...
    def forward(self, x):
        x = x.to(self.device)  # 将输入张量移动到 GPU
        x = self.conv1(x)
...
def meta_resnet(channels, num_classes=1000, include_top=True):
    groups = 64
    width_per_group = 3
    return ResNet(block=BasicBlock, blocks_num=[3, 4, 6, 3],
